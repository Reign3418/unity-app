document.addEventListener('DOMContentLoaded', () => {
    ...
    const elements = {
        ...
        npwdKingdomSelect: document.getElementById('npwdKingdomSelect'),
        ...
        rankingGovCountSelect: document.getElementById('rankingGovCountSelect')
    };
    ...
    function setupFileUpload(inputId, zoneId, type, successMsg) {
        const input = document.getElementById(inputId);
        const zone = document.getElementById(zoneId);

        if (!input || !zone) return;

        // Click to browse - Handled by CSS overlay (input covers zone)
        // zone.addEventListener('click', ...); RE MOVED to prevent double triggers
        ...
        zone.addEventListener('drop', (e) => {
            ...
            handleFiles(e.dataTransfer.files, type, successMsg);
        });

        // File Input Change
        input.addEventListener('change', (e) => handleFiles(e.target.files, type, successMsg));
    }

    async function handleFiles(fileList, type, successMsg) {
        ...
        try {
            ...
            results.forEach(result => {
                ...
                // Update Date & UI
                if (type === 'start') {
                    AppState.startScanDate = date;
                    updateScanDetails('start', date, kingdoms);
                } else if (type === 'mid') {
                    AppState.midScanDate = date;
                    updateScanDetails('mid', date, kingdoms);
                } else {
                    AppState.endScanDate = date;
                    updateScanDetails('end', date, kingdoms);
                }
                ...
                kingdoms.forEach(kId => {
                    ...
                    // Legacy Support: Ensure arrays exist (for saved states from before this update)
                    if (!AppState.kingdoms[kId].midData) AppState.kingdoms[kId].midData = [];
                    if (!AppState.kingdoms[kId].startData) AppState.kingdoms[kId].startData = [];
                    if (!AppState.kingdoms[kId].endData) AppState.kingdoms[kId].endData = [];

                    if (type === 'start') AppState.kingdoms[kId].startData.push(...data.filter(r => r['_kingdom'] === kId));
                    else if (type === 'mid') AppState.kingdoms[kId].midData.push(...data.filter(r => r['_kingdom'] === kId));
                    else AppState.kingdoms[kId].endData.push(...data.filter(r => r['_kingdom'] === kId));
                });
                filesLoaded++;
            });
            ...
            // Recalculate all loaded kingdoms
            AppState.loadedKingdoms.forEach(kId => calculateKingdom(kId));
            renderKingdomComparison();

            // alert(`${successMsg} (${filesLoaded} files)`); // Optional: Reduce spam
        } catch (error) {
            console.error(error);
    ...
    // Call setup in DOMContentLoaded (assuming this block is replacing the old listeners)
    setupFileUpload('startScanInput', 'startScanZone', 'start', 'Start Scan Loaded');
    setupFileUpload('midScanInput', 'midScanZone', 'mid', 'Mid Scan Loaded');
    setupFileUpload('endScanInput', 'endScanZone', 'end', 'End Scan Loaded');
    ...
    function createKingdomState() {
        return {
            startData: [],
            midData: [], // Fix: Initialize midData
            endData: [],
            calculatedData: [],
            currentOverviewData: [],
            config: {
    ...
    function switchSubTab(kingdomId, subTabId) {
        ...
        if (subTabId === 'overview') syncOverviewScroll(kingdomId);
            ...
            if (data.length > 0) renderRosterAnalysis(kingdomId, data);
            else if (document.getElementById('rosterInsights')) {
                document.getElementById('rosterInsights').innerHTML = '<p>No data available for analysis.</p>';
            }
        }
    }
    ...
    function switchPrekvkSubTab(subTabId) {
        ...
        // Kingdom Select Visibility
        if (select) {
            // Hide Kingdom Select for main kingdom summary, show for drill-downs
            if (subTabId === 'kingdom-analysis') select.classList.add('hidden');
            else select.classList.remove('hidden');
        }

        // Gov Count Visibility
        if (govCountSelect) {
            // Show only for kingdom analysis now, or maybe for others too? 
            // Original logic: Show for kingdom-analysis, hide for others.
            // User Request "can we add a kingdom selection to see all the alliance in each kingdom"
            // implies they want to drill down.
    ...
    if (elements.prekvkKingdomSelect) {
        elements.prekvkKingdomSelect.addEventListener('change', (e) => {
            ...
            if (activeSubTab) {
                ...
                if (subTabId === 'alliance-analysis') renderAllianceAnalysis(e.target.value);
                if (subTabId === 'governor-analysis') renderGovernorAnalysis(e.target.value);
            }
        });
    }

    if (elements.prekvkGovCountSelect) {
        elements.prekvkGovCountSelect.addEventListener('change', () => {
            // Only impacts kingdom analysis based on current logic
            renderKingdomAnalysis();
        });
    }
    ...
    function renderKingdomAnalysis() {
        ...
        AppState.loadedKingdoms.forEach(kId => {
            ...
            let processedData = getFilteredData([...sourceData]);
        ...
        // Default Sort: Total Power Descending
    ...
    function createKingdomContent(kingdomId) {
        ...
        const kConfig = AppState.kingdoms[kingdomId].config;
        ...
        if (logToggle && limitSelect) {
            const updateRoster = () => {
                ...
                const data = kState.startData.length > 0 ? kState.startData : kState.endData;
                if (data.length > 0) {
                    renderRosterAnalysis(kingdomId, data, logToggle.checked, parseInt(limitSelect.value));
                }
            };
    ...
    function calculateKingdom(kingdomId) {
        ...
        if (kState.startData.length === 0 || kState.endData.length === 0) {
            // Silently return if data is incomplete - UI will show status
            return;
        }

        const config = kState.config;
        const midData = kState.midData; // Fix: Define local variable
        // Use filtered data for calculation
        const startFiltered = getFilteredData(kState.startData);
        const midFiltered = getFilteredData(kState.midData);
        const endFiltered = getFilteredData(kState.endData);
        ...
        allIds.forEach(id => {
            ...
            // Healed Logic: Prioritize End - Mid (Troop Power Gain)
            // Fallback: Use 'Healed' column if available diff
            let healedDiff = 0;
            if (midData.length > 0) {
                // Healed = Growth from Low Point (Mid) to End
                // Assumption: Troop Power gain during recovery phase is mostly healing
                const endTroop = parseNumber(endRow['Troop Power']);
                const midTroop = parseNumber(midRow['Troop Power']);
                if (endTroop > 0 && midTroop > 0) {
                    healedDiff = Math.max(0, endTroop - midTroop);
                }
            } else {
                healedDiff = Math.max(0, parseNumber(endRow['Healed']) - parseNumber(startRow['Healed']));
            }
        ...
        if (container) {
            ...
            if (resultsSection) {
                ...
                console.error(`Results section for kingdom ${kingdomId} not found.`);
            }
    ...
    function renderResultsTable(kingdomId, data) {
        const container = document.getElementById(`kingdom-${kingdomId}`);
        if (!container) {
            console.error(`Container for kingdom ${kingdomId} not found.`);
            return;
        }
        const tbody = container.querySelector('.dkp-table tbody');
        if (!tbody) {
            console.error(`Results table body for kingdom ${kingdomId} not found.`);
            return;
        }
        const fragment = document.createDocumentFragment();
        tbody.innerHTML = '';
    ...
    function renderScatterChart(kingdomId, overviewData) {
        ...
        const getAdjustedKP = (row) => {
            if (row.kvkKP !== undefined) return row.kvkKP; // Already calculated
            const t4 = parseNumber(row['T4 Kills']);
            const t5 = parseNumber(row['T5 Kills']);
        ...
        const points = validRows.map((row, index) => {
            const kp = getAdjustedKP(row);
            const deads = row.deads !== undefined ? row.deads : parseNumber(row['Deads']);
            const startPower = row.startPower !== undefined ? row.startPower : parseNumber(row['Power']);
            const id = row['Governor ID'] || row.id;
            const name = row['Governor Name'] || row.name || 'Unknown';

            let color = '#9ca3af'; // Slacker
            if (kp >= avgKP && deads < avgDeads) color = '#10b981'; // Hero
            else if (kp >= avgKP && deads >= avgDeads) color = '#facc15'; // Warrior
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: (context) => {
                                const p = context.raw;
                                return `${p.name}(ID: ${p.id}) | T4/T5 KP: ${p.rawKP.toLocaleString()} | Deads: ${p.deads.toLocaleString()}`;
                            }
                        }
                    }
                },
    ...
    function renderRosterAnalysis(kingdomId, data, useLogScale = false, limit = 300) {
        ...
        // Use filtered data
        const validPlayers = getFilteredData(data).filter(p => parseNumber(p.Power) > 1000000);
        ...
        limitedPlayers.forEach(p => {
            const t4 = parseNumber(p['T4 Kills']);
            const t5 = parseNumber(p['T5 Kills']);
            // STRICTLY T4+T5, ignore 'Kill Points' column which has T1-T3
    ...
    // --- New Phone Who Dis Logic ---

    if (elements.npwdKingdomSelect) {
        elements.npwdKingdomSelect.addEventListener('change', (e) => {
            renderNewPhoneWhoDis(e.target.value);
        });
    }

    function updateNPWDDropdown() {
        const select = elements.npwdKingdomSelect;
        if (!select) return;
        select.innerHTML = '';
        Array.from(AppState.loadedKingdoms).forEach(kId => {
            const option = document.createElement('option');
        ...
        if (AppState.loadedKingdoms.size > 0 && select.value === '') {
            select.value = Array.from(AppState.loadedKingdoms)[0];
            renderNewPhoneWhoDis(select.value);
        }
    }

    function renderNewPhoneWhoDis(kingdomId) {
        ...
        // New Arrivals: In End but NOT in Start
        const newArrivals = kState.endData.filter(r => !startIds.has(r['Governor ID']));

        // Departures: In Start but NOT in End
        ...
        const formatData = (list) => list.map(r => ({
            ...
            'Power': parseNumber(r['Power'])
        })).sort((a, b) => b.Power - a.Power);

        renderAnalysisTable(formatData(newArrivals), newContainer);
        renderAnalysisTable(formatData(departures), departContainer);
    }

    // --- PreKVK Ranking Logic ---

    if (elements.rankingGovCountSelect) {
        elements.rankingGovCountSelect.addEventListener('change', (e) => {
            document.getElementById('rankingCountLabel').textContent = e.target.value === 'all' ? 'All' : e.target.value;
            renderPreKVKRanking();
        });
    }

    function renderPreKVKRanking() {
        const tbody = document.querySelector('#rankingTable tbody');
        if (!tbody) return;
        tbody.innerHTML = '';

        const limitStr = elements.rankingGovCountSelect ? elements.rankingGovCountSelect.value : '300';
        ...
        AppState.loadedKingdoms.forEach(kId => {
            ...
            // Strict Start Scan usage as requested
            let validGovs = kState.startData.map(r => ({
                power: parseNumber(r['Power']),
                kp: parseNumber(r['Kill Points'])
            })).sort((a, b) => b.power - a.power);

            if (limit !== Infinity) {
                validGovs = validGovs.slice(0, limit);
            }

            const totalPower = validGovs.reduce((sum, g) => sum + g.power, 0);
            const totalKP = validGovs.reduce((sum, g) => sum + g.kp, 0);
        ...
        // Rank based on KP
        // We need a separate sort to find KP ranks, effectively we map KD to rank
        const kpSorted = [...rankingData].sort((a, b) => b.kp - a.kp);
        const kpRankMap = new Map();
        kpSorted.forEach((d, i) => kpRankMap.set(d.kingdom, i + 1));

        // Assign KP Ranks and Final Sort (Default Power)
        rankingData.forEach(d => d.kpRank = kpRankMap.get(d.kingdom));

        // Final Sort based on User Selection
        const { key, dir } = AppState.rankingSort || { key: 'power', dir: 'desc' };
        ...
        // Add Header Listeners if not exists
        const headers = document.querySelectorAll('#rankingTable th[data-sort]');
        headers.forEach(th => {
            if (!th.dataset.hasListener) {
                th.addEventListener('click', () => {
                    const sortKey = th.dataset.sort;
                    // Toggle dir if same key, else default desc (except ranks/kingdom which default asc)
                    const currentSort = AppState.rankingSort || { key: 'power', dir: 'desc' };
                    ...
                    AppState.rankingSort = currentSort;
                    renderPreKVKRanking();
                });
    ...
    function saveState() {
        try {
            ...
            localStorage.setItem('dkp_scanDates', JSON.stringify({ start: AppState.startScanDate, mid: AppState.midScanDate, end: AppState.endScanDate }));
        } catch (e) { console.error('Failed to save state:', e); }
    }

    function updateScanDetails(type, date, kingdoms) {
        let detailsEl;
        if (type === 'start') detailsEl = elements.startScanDetails;
        else if (type === 'mid') detailsEl = document.getElementById('midScanDetails');
        else detailsEl = elements.endScanDetails;
    ...
    function loadState() {
        try {
            ...
            if (savedDates) {
                ...
                AppState.startScanDate = dates.start;
                AppState.midScanDate = dates.mid;
                AppState.endScanDate = dates.end;
            }
            if (AppState.loadedKingdoms.size > 0) {
                renderKingdomTabs();
                ...
                if (firstK) {
                    ...
                    if (firstK.midData && firstK.midData.length > 0) updateScanDetails('mid', AppState.midScanDate, Array.from(AppState.loadedKingdoms));
