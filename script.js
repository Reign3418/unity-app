...
class GitHubService {
    constructor() {
        ...
        this.config = JSON.parse(localStorage.getItem('unity_gh_config')) || { owner: '', repo: '', token: '' };
    }
    ...
    async getFileContent(path) {
        const url = `${this.baseUrl}/repos/${this.config.owner}/${this.config.repo}/contents/${path}`;
        const response = await fetch(url, { headers: this.getHeaders() });
        if (!response.ok) throw new Error(`Could not fetch file metadata: ${response.statusText}`);
        const data = await response.json();

        // Use download_url if available (handles large files and avoids Base64 decoding issues)
        if (data.download_url) {
            const rawResponse = await fetch(data.download_url);
            if (!rawResponse.ok) throw new Error(`Could not fetch raw content: ${rawResponse.statusText}`);
            return await rawResponse.json();
        }

        // Fallback for small files if download_url is missing
        if (data.content) {
                ...
                loadBtn.addEventListener('click', async () => {
                    ...
                    if (val.startsWith('cloud:')) {
                        try {
                            ...
                            this.data.processImportedData(content, type);
