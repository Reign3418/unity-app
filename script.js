    ...
    static calculateKingdom(kingdomId, state) {
        ...
        allIds.forEach(id => {
            ...
            kState.calculatedData.push({
                ...
                bonus: 0 // Bonus restored from state if ID matches? Current implementation resets bonuses on recalc. 
                // Refinement: Ideally we should preserve bonuses.
                // existingData.find(r => r.id === id)?.bonus || 0;
            });
        });
...
class UIService {
    ...
    cacheElements() {
        ...
        const ids = [
            'mainTabs', 'kingdomsContainer', 'kingdomContentTemplate',
    ...
    switchKingdom(kingdomId) {
        ...
        const btn = this.elements.mainTabs.querySelector(`[data-tab="kingdom-${kingdomId}"]`);
    ...
    renderKingdomTabs() {
        ...
        this.elements.mainTabs.querySelectorAll('[data-tab^="kingdom-"]').forEach(t => t.remove());
        ...
        this.data.state.loadedKingdoms.forEach(kId => {
            ...
            this.elements.mainTabs.appendChild(btn);
    ...
    createKingdomContent(kingdomId) {
        ...
        if (calcBtn) calcBtn.addEventListener('click', () => CalculationService.calculateKingdom(kingdomId, this.data.state));
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const handleFiles = async (fileList, type, successMsg) => {
        ...
        try {
            ...
            dataService.state.loadedKingdoms.forEach(kId => CalculationService.calculateKingdom(kId, dataService.state));
