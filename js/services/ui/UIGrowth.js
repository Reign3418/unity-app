...
const GROWTH_CONFIG = {
    ...
    GRADES: { S: 80, A: 60, B: 40, C: 20 },
    ARCHETYPES: {
        KP_THRESHOLD: 0.4,
        DEADS_THRESHOLD: 0.4,
        FARM_THRESHOLD: 0.5
    },
    CAP: 1.25 // Max weight overflow
};
...
class UIGrowth {
    ...
    // ... (rest of methods up to calculateGrowthScores) ...
    ...
    renderTable(kingdomId, data) {
        ...
        data.forEach(p => {
            ...
            const safeAlliance = Utils.escapeHtml(p.alliance);

            tr.innerHTML = `
                <td>${safeName}</td>
                <td><span class="alliance-tag">${safeAlliance}</span></td>
                ...
                <td>
                    <button class="primary-btn mail-btn-small" style="padding: 0.3rem 0.6rem; font-size: 0.8rem; border-radius: 4px;" data-id="${p.id}">✉️ Mail</button>
                </td>
            `;

            // Attach Mail Generator Listener
            tr.querySelector('.mail-btn-small').addEventListener('click', () => {
                this.sendIndividualToMailGenerator(p);
            });
    ...
    calculateGrowthScores(startData, endData) {
        ...
        // Identify Percentile Baselines first? 
        // No, we need deltas first.
        // We do 2 passes. 1 to calculate deltas, 1 to score.
        // Actually the original code did it in one pass of `processed` after building it.
        ...
        const W = GROWTH_CONFIG.WEIGHTS;
        const CAP = GROWTH_CONFIG.CAP;

        processed.forEach(p => {
            const sTech = Math.min(CAP, p.techDelta / maxTech) * W.TECH;
            const sBld = Math.min(CAP, p.bldDelta / maxBld) * W.BLD;
            const sTroop = Math.min(CAP, p.troopGrowth / maxTroop) * W.TROOP;

            const sKP = Math.min(CAP, p.kpDelta / maxKP) * W.KP;
            const sDeads = Math.min(CAP, p.deadsDelta / maxDeads) * W.DEADS;

            const sGathered = Math.min(CAP, p.gatheredDelta / maxGathered) * W.GATHERED;
    ...
    determineArchetype(p, maxKP, maxDeads, maxGathered) {
        // Simple heuristic
        const T = GROWTH_CONFIG.ARCHETYPES;

        // Dynamic Thresholds passed in are 1.5x the baseline. 
        // We use relative check against the passed maxes? 
        // The original code passed `maxKP * 1.5` as `maxKP`.
        // So `p.kpDelta > maxKP * 0.4` means `p.kpDelta > (realMax * 1.5) * 0.4` = `realMax * 0.6`.
        // Let's keep logic same but use constant.

        const isHighKP = p.kpDelta > (maxKP * T.KP_THRESHOLD);
        const isHighDeads = p.deadsDelta > (maxDeads * T.DEADS_THRESHOLD);
        const isHighFarm = p.gatheredDelta > (maxGathered * T.FARM_THRESHOLD);
    ...
    sendIndividualToMailGenerator(p) {
        const fmt = Utils.formatCompactNumber;
        const mailDateRange = this.reportDateRange || "";

        let report = `<size=30><b><color=#8B5CF6>Unity Growth Report</color></b></size>\n`;
        report += `<color=#A9A9A9>Scan Period: ${mailDateRange}</color>\n\n`;

        report += `<size=24><color=#000000><b>${p.name}</b></color> [${p.alliance}]</size>\n`;
        report += `Grade: <color=#32CD32><b>[${p.grade}]</b></color> (Score: ${p.finalScore.toFixed(1)})\n`;
        report += `Archetype: <color=#3b82f6>${p.archetype}</color>\n\n`;

        report += `<size=22><b>Performance Deltas</b></size>\n`;
        report += `Power: <color=${p.powerDelta >= 0 ? '#32CD32' : '#FF4500'}>${p.powerDelta > 0 ? '+' : ''}${fmt(p.powerDelta)}</color>\n`;
        report += `Kill Points: <color=#32CD32>+${fmt(p.kpDelta)}</color>\n`;
        report += `Deads: <color=#FF4500>+${fmt(p.deadsDelta)}</color>\n`;
        report += `Gathered: <color=#D2691E>${fmt(p.gatheredDelta)}</color>\n\n`;

        report += `<i>Detailed Growth Info</i>\n`;
        report += `Troop Pwr: ${fmt(p.troopDelta)}\n`;
        report += `Tech Pwr: ${fmt(p.techDelta)}\n`;
        report += `Cmdr Pwr: ${fmt(p.cmdDelta)}\n`;
        report += `Bld Pwr: ${fmt(p.bldDelta)}\n`;
        report += `Assistance: ${fmt(p.rssDelta)}\n\n`;
        report += `Generated by Unity`;
        ...
        if (mailInput && mailTabBtn) {
            ...
            console.error('Mail Generator interface elements not found.');
        }
    }
