                ...
                reader.onload = (e) => {
                    try {
                        ...
                        if (Array.isArray(json)) {
                            ...
                            let dateMatch = name.match(/(\d{4}-\d{2}-\d{2}T[\d-]+Z)/);

                            if (dateMatch) {
                                try {
                                    // Parse Strict ISO-like format
                                    const raw = dateMatch[1];
                                    date = raw.split('T')[0];
                                    console.log(`[DataService] Extracted ISO Date from filename: ${date}`);
                                } catch (e) { console.warn(e); }
                            } else {
                                // Fallback: Simple YYYY-MM-DD
                                dateMatch = name.match(/(\d{4}-\d{2}-\d{2})/);
                                if (dateMatch) {
                                    date = dateMatch[1];
                                    console.log(`[DataService] Extracted Simple Date from filename: ${date}`);
                                }
                            }
                            ...
                            // Fallback to filename if date missing in object
                            if (!date) {
                                const name = file.name;
                                let dateMatch = name.match(/(\d{4}-\d{2}-\d{2}T[\d-]+Z)/);
                                if (dateMatch) {
                                    try {
                                        ...
                                        console.log(`[DataService] Extracted ISO Date from filename (Obj Fallback): ${date}`);
                                    } catch (e) { }
                                } else {
                                    dateMatch = name.match(/(\d{4}-\d{2}-\d{2})/);
                                    if (dateMatch) {
                                        date = dateMatch[1];
                                        console.log(`[DataService] Extracted Simple Date from filename (Obj Fallback): ${date}`);
                                    }
                                }
                            }
